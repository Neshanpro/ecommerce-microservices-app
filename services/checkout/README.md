...
The Checkout Service is an orchestrator that requires downstream service addresses at startup. For smoke testing with mock backends:
# Run the optimized container with required environment variables
docker run -d \
  --name checkout-test \
  -p 5050:5050 \
